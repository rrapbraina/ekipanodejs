...
router.get('/', async(req, res) => {
    ...
    const customers = await Customer.find({isGold: true}).sort('name');
    res.send(customers);

    if(customers.length === 0){
        return res.status(404).send('There is no customer available')
    }
})
...
router.put('/:id', async(req, res) => {
    ...
    if(!customer) return res.status(404).send('Customer with the given ID is not available');
    res.send(customer);
});
